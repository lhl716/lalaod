...
class Knowledge_Base:
    def __init__(self, 
        ...
        self._update_mask_generator()  # 新增初始化方法
    ...
    def update_segmentation_params(self, new_params: dict):
        """
        动态更新分割参数并重新初始化分割器
        :param new_params: 需要更新的参数字典，例如 {"points_per_side": 32}
        """
        # 合并新旧参数
    ...
    def visualize_segmentation(self, 
        ...
        :param save_path: 保存路径（None则显示不保存）
        :param max_cols: 每行最多显示列数
        :param figsize: 图像尺寸（英寸）
        ...
        plt.suptitle(f"SAM Segmentation Results - {os.path.basename(image_path)}", 
                    fontsize=12, y=0.95)
        ...
        for i, patch in enumerate(patches, 1):
            ...
            title = [
                f"Patch {i}",
                f"Size: {patch.size[0]}x{patch.size[1]}"
            ]
            ax.set_title("\n".join(title), 
                        fontsize=fontsize,
                        color='#2c3e50',
...
if __name__ == "__main__":
    ...
    for i, params in enumerate(configs, 1):
        # 应用新参数
        kb.seg_params.update(params)
        
        # 生成可视化结果
        kb.visualize_segmentation(
            image_path="/data2/lihl/data/VOCdevkit/VOC2007/JPEGImages/000214.jpg",
            save_path=f"/data/lihl/fsod/model/knowledge_base/debug_sam/seg_config_{i}.png",
            max_cols=8,
            figsize=(20, 15)
        )
